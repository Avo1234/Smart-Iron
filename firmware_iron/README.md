...
This directory contains the modular ESP32-WROOM-32 firmware for the Smart Iron project. It manages local touchscreen controls, temperature regulation, handle-presence sensing, impact detection, alarms, fault recovery, and the BLE interface for the planned mobile application.
...
The firmware has been source-compiled against ESP32 Arduino core 3.3.11 and the installed dependency versions. In the development environment, the final linker process stalled without reporting a further source error, so a complete local binary should still be confirmed before upload.
