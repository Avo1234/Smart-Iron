...
- Make iron status and safe control functions available to a future mobile app over BLE.
...
XPT2046 touchscreen <----> ILI9341 TFT ----+      +<-> BLE <-> Future mobile app
                                                   +--> LEDs and buzzer
```
...
`-- firmware_iron/
    |-- README.md          # Firmware wiring, behavior, BLE, and build guide
    |-- firmware_iron.ino  # Setup, loop, and system orchestration
    |-- config.h           # Pins, limits, timing, and BLE UUIDs
    |-- app_state.h        # Shared state, actions, and shutdown reasons
    `-- *_controller.*     # Temperature, safety, display, BLE, and alarms
```
...
4. Implement the cross-platform BLE mobile application.
5. Test BLE commands, disconnections, status notifications, and safety restrictions with the mobile app.
6. Conduct controlled system testing followed by repeat-cycle reliability testing.
